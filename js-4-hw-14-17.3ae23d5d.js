...
module.bundle.HMR_BUNDLE_ID = "3840807d3ae23d5d";
"use strict";
...
function renderCountryList(countries) {
    results.innerHTML = countries.map((c)=>`<li>${c.name.common}</li>`).join("");
}
...
async function fetchCountries() {
    ...
    try {
        ...
        } else if (data.length > 1) renderCountryList(data);
        else if (data.length === 1) renderCountryDetails(data[0]);
    } catch (e) {
        results.innerHTML = "";
...
let catPage = 1;
const catLimit = 4;
const catApiKey = "live_MbqbGmV1SsALeirOBY5Mt36UjMlvxDqG1RtzxSk0DAmOMH9zS7A8ehMiZBHNstnA";
async function loadCats() {
    try {
        const res = await fetch(`https://api.thecatapi.com/v1/images/search?limit=${catLimit}&page=${catPage}&order=Desc&category_ids=1`, {
            headers: {
                "x-api-key": catApiKey
            }
        });
